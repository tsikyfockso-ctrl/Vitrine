...
MOTS_CLES_RECHERCHE = ["Children Clothing", "Kids Clothes", "Baby Clothes", "Boys Clothing", "Girls Clothing"]
...
def generate_update_stock_enfant_json():
    token = get_cj_access_token()
    
    # 1. Charger l'ancien fichier JSON existant pour préserver les produits ayant encore du stock
    produits_existants = {}
    if os.path.exists("update_stock_enfant.json"):
        try:
            with open("update_stock_enfant.json", "r", encoding="utf-8") as f:
    ...
    if not token:
        ...
        if produits_existants:
            with open("update_stock_enfant.json", "w", encoding="utf-8") as f:
    ...
    print(f"🎉 Succès global : {len(resultat_final)} produits actifs enregistrés dans update_stock_enfant.json")

if __name__ == "__main__":
    generate_update_stock_enfant_json()
