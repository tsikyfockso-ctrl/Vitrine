// ==========================================
// CONFIGURATION INITIALE DE LA VITRINE
// ==========================================
window.onload = async () => {
...
// --- GESTION DE LA MARGE COMMERCIALE ADMIN ---
...
// --- AFFICHAGE DES PRODUITS PAR CONTENEUR ---
function renderCategoryProducts(stock, container) {
    ...
    container.innerHTML = stock.map((p, index) => {
        ...
        // Application automatique de la marge admin sur le prix affiché
        let prixAffiche = calculerPrixVitrine(prixBrut);
...
// --- GESTION DES MODALES, PAYS ET PAIEMENTS ---
    ...
    if (variantSelect) {
        ...
        if (Array.isArray(currentSelectedProduct.variantes)) {
            ...
            currentSelectedProduct.variantes.forEach((v, i) => {
                ...
                let prixVarFinal = calculerPrixVitrine(prixVarBrut);
...
function calculateShipping() {
    ...
    let currentPriceFinal = parseFloat(calculerPrixVitrine(currentPriceBrut)) * quantite;
    ...
    if (variantSelect) {
        Array.from(variantSelect.options).forEach((opt, i) => {
            ...
            if (v) {
                let pFinal = calculerPrixVitrine(v.prix || 0);
