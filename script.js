...
function renderProducts(products, container) {
    ...
    products.forEach((product, index) => {
        ...
        // Proxy d'images pour contourner les blocages stricts (AliExpress / CJ)
        ...
        card.style.cssText = "background: #fff; border-radius: 8px; padding: 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); cursor: pointer; text-align: center;";
        
        card.innerHTML = `
            <img src="${imageAffichage}" alt="${product.nom}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 6px;">
            <h3 style="font-size: 15px; margin: 10px 0; color: #333; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${product.nom}</h3>
            <p style="font-size: 16px; font-weight: bold; color: #e11d48; margin-bottom: 10px;">${prixAffichage} €</p>
            <button style="width: 100%; background-color: #0f172a; color: white; border: none; padding: 10px; border-radius: 5px; cursor: pointer; font-weight: bold;">Voir le produit</button>
        `;
...
function openProductModal(productIndex) {
    ...
    if (!modal) {
        console.error("Erreur : La modale HTML est introuvable sur la page.");
        return;
    }
    ...
    // Gestion de l'image principale dans la modale
    let imagesTab = Array.isArray(product.images) ? product.images : [product.images];
    ...
    // --- INTEGRATION DU BLOC DELAI DE LIVRAISON AVEC INFO-BULLE ET POINT D'INTERROGATION ---
    let deliveryContainer = document.getElementById('modalDeliverySection');
    if (!deliveryContainer) {
        // S'il n'existe pas dans le HTML d'origine, on le crée dynamiquement sous les détails
...
function initialiserPays() {
    ...
    paysDisponibles.forEach(p => {
        ...
        if (p.code === "FR") opt.selected = true; // Par défaut France
...
function calculateShipping() {
    ...
    // Multiplicateurs selon les zones géographiques
    ...
    // Récupération de la base calculée par Python (Ligne de liquide + surplus 0.99€)
    let shippingBaseProduit = currentSelectedProduct.shippingBase !== undefined 
        ? parseFloat(currentSelectedProduct.shippingBase) 
        : 4.53; // Valeur de repli sécurisée
    ...
    // Calcul du prix total (Article + Transport)
