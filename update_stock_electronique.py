...
MOTS_CLES_RECHERCHE = ["Consumer Electronics", "Gadgets", "Phone Chargers", "Earphones", "Bluetooth Speakers", "Phone"]
...
def generate_update_stock_electronique_json():
    token = get_cj_access_token()
    
    # 1. Charger l'ancien fichier JSON existant pour préserver les produits ayant encore du stock
    produits_existants = {}
    if os.path.exists("update_stock_electronique.json"):
        try:
            with open("update_stock_electronique.json", "r", encoding="utf-8") as f:
    ...
    if not token:
        ...
        if produits_existants:
            with open("update_stock_electronique.json", "w", encoding="utf-8") as f:
    ...
    print(f"🎉 Succès global : {len(resultat_final)} produits actifs enregistrés dans update_stock_electronique.json")

if __name__ == "__main__":
    generate_update_stock_electronique_json()
