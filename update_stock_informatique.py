...
MOTS_CLES_RECHERCHE = ["Computer accessory", "Keyboards", "desktop computer", "Laptops", "USB Hubs","Networking"]
...
def generate_update_stock_informatique_json():
    ...
    if os.path.exists("update_stock_informatique.json"):
        try:
            with open("update_stock_informatique.json", "r", encoding="utf-8") as f:
    ...
    print(f"🎉 Succès global : {len(resultat_final)} produits actifs enregistrés dans update_stock_informatique.json")

if __name__ == "__main__":
    generate_update_stock_informatique_json()
