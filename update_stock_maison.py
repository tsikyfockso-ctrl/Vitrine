...
MOTS_CLES_RECHERCHE = ["Home & Garden", "Furniture", "Home Decor", "Cushions", "Home Lighting", "Home","console tables"]
...
def generate_update_stock_maison_json():
    token = get_cj_access_token()
    
    # 1. Charger l'ancien fichier JSON existant pour préserver les produits ayant encore du stock
    produits_existants = {}
    if os.path.exists("update_stock_maison.json"):
        try:
            with open("update_stock_maison.json", "r", encoding="utf-8") as f:
    ...
    if not token:
        ...
        if produits_existants:
            with open("update_stock_maison.json", "w", encoding="utf-8") as f:
    ...
    print(f"🎉 Succès global : {len(resultat_final)} produits actifs enregistrés dans update_stock_maison.json")

if __name__ == "__main__":
    generate_update_stock_maison_json()
